...
const { user, rol, loading, logout, deleteAccount } = useAuth();
...
{/* AVATAR */}
{user.photoURL ? (
<img src={user.photoURL} className="w-10 h-10 rounded-full border-2 border-[#d4af37]" />
) : (
...
<button onClick={()=>navigate("/admin")} className="text-sm hover:text-blue-400 uppercase px-4 py-2">
Panel Admin
</button>

<button onClick={logout} className="text-sm hover:text-red-400 px-4 py-2">
Cerrar Sesión
</button>

<button onClick={handleDeleteAccount} className="text-sm text-red-400 px-4 py-2">
Eliminar Cuenta
</button>
</>
...
<button onClick={()=>navigate("/panel-barbero")} className="text-sm hover:text-gold uppercase px-4 py-2">
Mi Panel
</button>
...
<button onClick={()=>navigate("/mis-reservas")} className="text-sm hover:text-gold px-4 py-2">
Mis Reservas
</button>

<button onClick={()=>navigate("/reserva")} className="px-6 py-3 bg-gradient-to-r from-[#d4af37] to-[#b4941f] text-black rounded-full font-bold">
Reservar
</button>
...
<>
<button onClick={()=>navigate("/iniciarsesion")} className="text-sm hover:text-gold px-4 py-2">
Iniciar Sesión
</button>
...
<button onClick={handleDeleteAccount} className="text-red-400">Eliminar Cuenta</button>
</>
) : rol === "barbero" ? (
<>
<button onClick={()=>navigate("/panel-barbero")}>Mi Panel</button>
<button onClick={logout}>Cerrar Sesión</button>
<button onClick={handleDeleteAccount} className="text-red-400">Eliminar Cuenta</button>
</>
) : (
...
<>
<button onClick={()=>navigate("/iniciarsesion")}>Iniciar Sesión</button>
<button onClick={()=>navigate("/reserva")}>Reservar</button>
</>
)}
